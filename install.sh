...
sudo apt-get update
sudo apt-get install -y rsync silversearcher-ag file
...
cp $DOTFILES_PATH/git-pre-commit ~/dd/dd-source/.git/hooks/pre-commit;
chmod +x ~/dd/dd-source/.git/hooks/pre-commit;
